...
describe("verifierGrosseDepense", () => {
  ...
  it("ne notifie pas sans historique suffisant", async () => {
    mockTransactionAggregate.mockResolvedValue({ _avg: { montant: decimalFictif(1000) }, _count: 2 });
  ...
  it("ne notifie pas si le montant reste sous le multiplicateur", async () => {
    mockTransactionAggregate.mockResolvedValue({ _avg: { montant: decimalFictif(1000) }, _count: 10 });
...
describe("detecterAnomaliesCategoriePourPersonne", () => {
  ...
  it("ignore une catégorie sans historique suffisant", async () => {
    mockTransactionGroupBy.mockResolvedValue([{ categorieId: "cat1", _sum: { montant: decimalFictif(50000) } }]);
  ...
  it("notifie quand la dépense du mois dépasse largement la moyenne historique proratisée", async () => {
    mockTransactionGroupBy.mockResolvedValue([{ categorieId: "cat1", _sum: { montant: decimalFictif(100000) } }]);
